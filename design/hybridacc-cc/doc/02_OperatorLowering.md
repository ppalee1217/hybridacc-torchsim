...
```python
# 每 Group 包含的 SRAM 容量
group_linear_words = BANKS_PER_GROUP * BANK_DEPTH      # 3 × 4096 = 12288 words (BANK_DEPTH=4096)
group_capacity     = group_linear_words * BYTES_PER_BANK_WORD  # 12288 × 8 = 98304 bytes = 96 KB

# 每 Group 的地址空間（含 Parallel region）
group_span_words   = (BANKS_PER_GROUP + 1) * BANK_DEPTH  # 4 × 4096 = 16384 words
```
...
├──────────────────────┤ half_group_capacity (49152)
│  PONG buffer         │  wave_tile_bytes     ← DMA 同時搬運下一波資料到此區
├──────────────────────┤ group_capacity (98304)
└──────────────────────┘
...
```python
# Parallel mode 地址常數
parallel_base     = group_linear_words * BYTES_PER_BANK_WORD  # 12288 × 8 = 98304
bank_depth_bytes  = BANK_DEPTH * BYTES_PER_BANK_WORD          # 4096 × 8 = 32768
half_parallel     = bank_depth_bytes // 2                      # 16384

parallel_ping_base = parallel_base                   # 98304
parallel_pong_base = parallel_base + half_parallel   # 114688
```
...
├──────────────────────┤ half_parallel (16384)
│  Bank 0 PONG         │
├──────────────────────┤ bank_depth_bytes (32768)
│  Bank 1 PING         │  ← DMA 寫入 port 1 的資料
├──────────────────────┤ bank_depth_bytes + half_parallel (49152)
│  Bank 1 PONG         │
├──────────────────────┤ 2 × bank_depth_bytes (65536)
│  Bank 2 PING         │  ← DMA 寫入 port 2 的資料
├──────────────────────┤ 2 × bank_depth_bytes + half_parallel (81920)
│  Bank 2 PONG         │
├──────────────────────┤ group_capacity (98304)
└──────────────────────┘
...
├──────────────────────┤ parallel_pong_base (114688)
│  PONG buffer         │
├──────────────────────┤ parallel_base + bank_depth_bytes (131072)
└──────────────────────┘

AGU BASE_ADDR：PING = parallel_ping_base (98304), PONG = parallel_pong_base (114688)
每個 addr 提供 3 × 64-bit = 192-bit → 3 port 各得 64-bit
half_parallel 內可容納的 AGU 迭代數 = half_parallel / pkt_size = 2048
等效資料容量 = 2048 × 3 × 8 = 49152 bytes = half_group_capacity（與 Linear mode 相同）
```

> **容量等價性**：Parallel mode 的 half_parallel (16384 地址空間) 與 Linear mode 的 half_group_capacity (49152 地址空間) 可容納的**實際資料量相同**（均為 48 KB）。因此 tiling 搜尋的 `wave_tile_bytes ≤ half_cap` 約束在兩種模式下等價，搜尋演算法不需修改。
...
- `H_out=14`, `W_out=198`
- `group_capacity = 96 KB = 98304 bytes`
- `half_group_capacity = 48 KB = 49152 bytes`
...
| Group | Buffer | 計算 | Size | ≤ 48KB? |
|-------|--------|------|------|---------|
| 0 (PS) | Weight | `16 × 3 × 3 × 1 × 8 = 1152` | 1.125 KB | ✓ |
| 1 (PD) | Input | `16 × 200 × 1 × 8 = 25600` | 25 KB | ✓ |
| 2 (PLI) | PSum | `14 × 198 × 4 × 8 = 88704` | 86.625 KB | ✗（> 48 KB） |
| 3 (PLO) | Output | `14 × 198 × 4 × 8 = 88704` | 86.625 KB | ✗（> 48 KB） |

PLI/PLO 超過 half_cap → 需要 spatial tiling。

搜尋 `tile_h_out` 向下：需要 `tile_h_out × tile_w_out × 4 × 8 ≤ 49152`，即 `tile_h_out × tile_w_out ≤ 1536`。

嘗試 `tile_h_out=7, tile_w_out=198`：`7 × 198 = 1386 ≤ 1536` ✓

| Group | Buffer | Size | ≤ 48KB? |
|-------|--------|------|---------|
| 0 (PS) | Weight | 1.125 KB | ✓ |
| 1 (PD) | Input | `9 × 200 × 1 × 8 = 14400` = 14.06 KB | ✓ |
| 2 (PLI) | PSum | `7 × 198 × 4 × 8 = 44352` = 43.3 KB | ✓ |
| 3 (PLO) | Output | 43.3 KB | ✓ |

**結論**：
- `tile_h_out=7, tile_w_out=198, tile_h_in=9, tile_w_in=200`
- `num_h_tiles = ceil(14/7) = 2`, `num_w_tiles = 1`
- `num_oc_tiles = 1`, `num_ic_tiles = 1`
- **Total waves = 1 × 2 × 1 × 1 = 2**
- 所有 Group 皆 ping-pong ✓

**SPM layout（per Group）**：
- Group 0 (PS): PING=0, PONG=49152, buffer=1152 bytes
- Group 1 (PD): PING=0, PONG=49152, buffer=14400 bytes
- Group 2 (PLI): PING=0, PONG=49152, buffer=44352 bytes
- Group 3 (PLO): PING=0, PONG=49152, buffer=44352 bytes

**Multi-cluster mapping**：
- 2 H tiles → 2 clusters active（各處理 1 H tile）
- 共用 weight（broadcast），各 cluster 搬不同 spatial 區域的 input
- 剩餘 2 clusters idle

**DMA 節省分析**：Weight（1.125 KB）broadcast 一次即可，不需各 cluster 重複搬運。
...
```
Per-Group Layout 範例（conv2d 3×3, tile_h_out=7, tile_w_out=198,
                       tile_ic=4, tile_oc=16, group_capacity=96KB）：

Group 0 (PS - Weight):                    Group 1 (PD - Activation):
┌──────────────────┐ 0                    ┌──────────────────┐ 0
│ PING: 1.125 KB   │                      │ PING: 14.06 KB   │
├──────────────────┤ 1152                  ├──────────────────┤ 14400
│ (unused 46.9 KB) │                      │ (unused 33.9 KB) │
╞══════════════════╡ 49152 (half_cap)     ╞══════════════════╡ 49152 (half_cap)
│ PONG: 1.125 KB   │                      │ PONG: 14.06 KB   │
├──────────────────┤ 50304                 ├──────────────────┤ 63552
│ (unused 46.9 KB) │                      │ (unused 33.9 KB) │
└──────────────────┘ 98304                 └──────────────────┘ 98304
...
┌──────────────────┐ 0                    ┌──────────────────┐ 0
│ PING: 43.3 KB    │                      │ PING: 43.3 KB    │
├──────────────────┤ 44352                 ├──────────────────┤ 44352
│ (unused 4.7 KB)  │                      │ (unused 4.7 KB)  │
╞══════════════════╡ 49152 (half_cap)     ╞══════════════════╡ 49152 (half_cap)
│ PONG: 43.3 KB    │                      │ PONG: 43.3 KB    │
├──────────────────┤ 93504                 ├──────────────────┤ 93504
│ (unused 4.7 KB)  │                      │ (unused 4.7 KB)  │
└──────────────────┘ 98304                 └──────────────────┘ 98304
```
...
**範例**（tile_ic=4, tile_oc=16, tile_h_out=7, tile_w_out=198）：
- `in_ch_pack = 4/4 = 1`
- `vectors_per_kernel = 3 × 1 = 3`
- `KERNEL_DMA_LEN = 16 × 3 = 48`
- `OUTPUT_WINDOW_CNT_MINUS_ONE = 7 × 198 - 1 = 1385`
- `KERNEL_COUNT = 16`
...
- Hardware：4 clusters, 64 PEs, 4 bus, SPM(BANKS_PER_GROUP=3, BANK_DEPTH=4096) → group_capacity=96KB, half_cap=48KB

**Step 1 — Tiling（4D: oc, h, w, ic）**：
1. 固定維度：`tile_ic=4`, `tile_oc=16`
2. `num_ic_tiles = 4/4 = 1`, `num_oc_tiles = 16/16 = 1`
3. 搜尋 tile_h_out × tile_w_out ≤ half_cap / (out_ch_pack × 8) = 49152 / (4 × 8) = 1536
4. 嘗試 `tile_h_out=7, tile_w_out=198`：7 × 198 = 1386 ≤ 1536 ✓
5. 驗證所有 Group ≤ half_cap（參照 §3.4.5）✓
6. `num_h_tiles = ceil(14/7) = 2`, `num_w_tiles = 1`
7. **Total waves = 1 × 2 × 1 × 1 = 2**

**Step 2 — SPM Per-Group Layout（強制 ping-pong）**：
- Group 0 (PS): PING=0, PONG=49152, wave=1152B
- Group 1 (PD): PING=0, PONG=49152, wave=14400B
- Group 2 (PLI): PING=0, PONG=49152, wave=44352B
- Group 3 (PLO): PING=0, PONG=49152, wave=44352B

**Step 3 — PE Template 參數**（使用 tiled 維度 tile_oc=16, tile_h_out=7, tile_w_out=198, tile_ic=4）：
- `in_ch_pack = 4/4 = 1`
- `KERNEL_DMA_LEN = 16 × 3 = 48`
- `OUTPUT_WINDOW_CNT_MINUS_ONE = 7 × 198 - 1 = 1385`
- `KERNEL_COUNT = 16`
...
- AGU PD: BASE=0, ITER01=(9<<16)|1=0x90001, ITER23=(1<<16)|200=0x100C8, STRIDE0=8, STRIDE1=1600, STRIDE2=8, STRIDE3=0
- AGU PLI: BASE=0, ITER01=(7<<16)|4=0x70004, ITER23=(1<<16)|198=0x100C6, STRIDE0=8, STRIDE1=6336, STRIDE2=32, STRIDE3=0
- AGU PLO: BASE=0（同 PLI 模式，指向 Group 3）

**Step 5 — Multi-cluster mapping**：
- 2 spatial H tiles → 2 clusters active（cluster 0: H tile 0, cluster 1: H tile 1）
- 共用 weight（broadcast to both clusters），各 cluster 搬不同 spatial 區域的 input
- 剩餘 2 clusters idle

**Step 6 — Temporal Loop（per-cluster, 4D）**：
```
# Cluster 0: H tile 0 (h_out rows [0,7))
# Cluster 1: H tile 1 (h_out rows [7,14))
buf = 0
for oc_tile in [0]:       # num_oc_tiles=1
  for h_tile in [0]:       # 每 cluster 只有 1 個 H tile
...
- Input: 14400/8 = 1800 transfers
- Output: 44352/8 = 5544 transfers
...
- `group_capacity = 96 KB`, `half_cap = 48 KB = 49152 bytes`
...
所有 Group ≤ 48 KB ✓ → 不需要 spatial tiling。
...
- Group 0 (PS): **Parallel** — PING=98304, PONG=114688, buffer=128B
- Group 1 (PD): Linear — PING=0, PONG=49152, buffer=8192B
- Group 2 (PLI): **Parallel** — PING=98304, PONG=114688, buffer=32768B
- Group 3 (PLO): **Parallel** — PING=98304, PONG=114688, buffer=32768B
...
|-------|--------|----------|----------|-----------|-----------|
| 0 (PS) | Weight | **Parallel** | **Ultra** | `parallel_ping_base` (98304) | `parallel_pong_base` (114688) |
| 1 (PD) | Activation | Linear | Normal | `0` | `half_cap` (49152) |
| 2 (PLI) | Partial Sum | **Parallel** | **Ultra** | `parallel_ping_base` (98304) | `parallel_pong_base` (114688) |
| 3 (PLO) | Output | **Parallel** | **Ultra** | `parallel_ping_base` (98304) | `parallel_pong_base` (114688) |
...
def compute_conv2d_1x1_spm_layout(
    ...
    parallel_ping = parallel_ping_base  # 98304
    parallel_pong = parallel_pong_base  # 114688
...
| Register | Value | 說明 |
|----------|-------|------|
| `BASE_ADDR` | `parallel_ping_base`（98304）或 `parallel_pong_base`（114688） | Parallel 地址範圍，隨 wave 交替 |
...
> - Bank 1（linear addr `bank_depth_bytes`=32768）：port 1 的 weight（channels 4–7）
> - Bank 2（linear addr `2×bank_depth_bytes`=65536）：port 2 的 weight（channels 8–11）
>
...
| Register | Value | 說明 |
|----------|-------|------|
| `BASE_ADDR` | `parallel_ping_base`（98304）或 `parallel_pong_base`（114688） | Parallel 地址範圍 |
...
| Group | Buffer | Wave tile size | 最大 tiles (half_cap=48KB) | 約束 |
|-------|--------|---------------|---------------------------|------|
| 0 (PS) | A tiles | 6 KB | 8 | `num_tiles × 6KB ≤ half_cap` |
| 1 (PD) | B tiles | 9 KB | 5 | `num_tiles × 9KB ≤ half_cap` |
| 3 (PLO) | C tiles | 3 KB | 16 | `num_tiles × 3KB ≤ half_cap` |
...
- `group_capacity = 96 KB = 98304 bytes`, `half_cap = 48 KB = 49152 bytes`
...
**SPM layout（per Group, 強制 ping-pong, PS/PLI/PLO = Parallel mode）**：
- Group 0 (PS): **Parallel** — PING=98304, PONG=114688
- Group 1 (PD): Linear — PING=0, PONG=49152
- Group 2 (PLI): **Parallel** — PING=98304, PONG=114688
- Group 3 (PLO): **Parallel** — PING=98304, PONG=114688
...
| Group | Buffer | SPM Mode | AGU Mode | Ping Base | Pong Base |
|-------|--------|----------|----------|-----------|-----------|
| 0 (PS) | A tile | **Parallel** | **Ultra** | `parallel_ping_base` (98304) | `parallel_pong_base` (114688) |
| 1 (PD) | B tile | Linear | Normal | `0` | `half_cap` (49152) |
| 2 (PLI) | C accum | **Parallel** | **Ultra** | `parallel_ping_base` (98304) | `parallel_pong_base` (114688) |
| 3 (PLO) | C out | **Parallel** | **Ultra** | `parallel_ping_base` (98304) | `parallel_pong_base` (114688) |
...
#### AGU PS（Matrix A，Group 0 — Parallel / Ultra）

A tile shape `[M_tile, K_tile]`。Ultra mode 下 3 port 各取得不同的 K 區段資料（見 AGU.md §5.2：每 port 對應不同 `k_tile`）。
...
> **DMA 載入**：DMA 需將 A tile 資料拆分寫入 3 個 Bank（各 Bank 存放分配給對應 port 的資料段落），使用 linear 地址：Bank 0 = addr 0, Bank 1 = addr 32768, Bank 2 = addr 65536。Ping-pong 半段偏移 = `half_parallel` (16384)。

#### AGU PD（Matrix B，Group 1 — Linear / Normal）

B tile shape `[K_tile, N_tile]`。PD 維持 Linear mode + Normal broadcast。
...
def validate_layer_hw_config(cfg: LayerHwConfig, hw: HardwareDesc):
    ...
    for group_name in ["ps", "pd", "pli", "plo"]:
        ...
        if layout.spm_mode == "parallel":
            assert layout.ping_base == parallel_ping_base  # 98304
            assert layout.pong_base == parallel_pong_base  # 114688
        else:
