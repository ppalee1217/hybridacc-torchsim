...
### 0.3 Multi-wave 設計決策（v2.1）

針對數千～數萬 wave 的 workload，本規格採用以下決策：

1. **Core 程式不展開每波**：改為固定 loop + descriptor table。
2. **Loader 指令一律 section 化**：scan chain 與 PE program 以 section id 載入，不在 core asm 內逐字展開。
3. **AGU/HDDU 採 profile + delta 更新**：首波 full config，後續波只更新 dynamic 欄位。
4. **以 code-size 優先**：接受 descriptor data 成長，避免 `SEC_CORE_PROGRAM` 線性膨脹。
...
- `HDDU.APPLY.DELTA profile_id, delta_ptr`：僅套用 dynamic 欄位（base/tag 等）
- `DMA.START.RANGE begin, count`：連續啟動 DMA descriptors
- `DMA.WAIT.MASK ch_mask, timeout_reg`：等待多通道 DMA
- `PE.LOAD.SECTION mX, pe_program_id`：由 section 載入指定 PE 指令流
- `NOC.SCAN.SECTION mX, scan_chain_id`：由 section 載入 scan chain words
...
- `WAVE.LOOP wave_base, wave_count`：可選巨集，展開成 loop 控制骨架
...
## 1.10 Multi-wave 精簡模式（v2.1 規範等級）

### MUST（第一版就需要）

1. `SEC_CORE_PROGRAM` 必須以「固定 loop + descriptor」方式生成，不得為每 wave 完整展開 AGU/MMIO 序列。
2. 必須支援 `WAVE.LOAD / WAVE.EXEC / WAVE.NEXT`，或語意等價的 microcode flow。
3. 必須支援 `PE.LOAD.SECTION` 與 `NOC.SCAN.SECTION`（可為 pseudo），避免把長資料常量塞入 core 指令區。
4. 必須將 HDDU/AGU 設定拆為 static 與 dynamic，並提供 delta 更新路徑（`HDDU.APPLY.DELTA` 或 compiler 等價 lowering）。

### SHOULD（第二階段建議）

1. 提供 profile cache（`last_profile_id`）減少重複 static writes。
2. `DMA.START.RANGE` 以硬體 sequencer 實作，降低 core 迴圈 overhead。
3. `SEC_WAVE_DESC` 支援 RLE/delta 壓縮。

### MAY（進階）

1. 支援 nest-loop descriptor（以 loop bounds + base formula 取代平面 wave list）。
2. 提供 `WAVE.LOOP` 單一 macro 指令，降低 compiler 產生的控制碼樣板複雜度。
...
6. `SEC_PROFILE_TABLE`：HDDU/AGU profile（含 static/dynamic 欄位切分 metadata）
7. `SEC_WAVE_DESC`：wave descriptor（建議用於 runtime loop 主資料）
8. `SEC_WAVE_TABLE`：可選別名/索引層，用於向後相容或工具鏈轉譯
...
5. 載入 `SEC_PROFILE_TABLE` + `SEC_WAVE_DESC` 到可存取記憶體（或 descriptor SRAM）
6. 啟動 `SEC_CORE_PROGRAM`
7. core program 依 `SEC_WAVE_DESC` loop 控制 DMA/cluster（`SEC_WAVE_TABLE` 可作映射層）
