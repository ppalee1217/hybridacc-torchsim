#ifndef HYBRIDACC_PE_VADD_HPP
#define HYBRIDACC_PE_VADD_HPP
...
SC_MODULE(VADD) {
    ...
    sc_out<sc_uint<T_DATA_WIDTH*T_VECTOR_SIZE>> vout_o; // Output vector result
    ...
    void comb_proc() {
        ...
        std::vector<DATA_TYPE> va = sc_uint_to_vector(va_i.read());
        std::vector<DATA_TYPE> vb = sc_uint_to_vector(vb_i.read());

        // Element-wise addition with vec_p (partial sums)
        std::vector<DATA_TYPE> add_result;

        add_result = vector_add(va, vb);
        DATA_TYPE reduced_result = adder_tree(va, p_i.read());
...
private:
    ...
    DATA_TYPE adder_tree(const std::vector<DATA_TYPE>& vec, DATA_TYPE initial_VADDe) { // reduction
        DATA_TYPE sum = initial_VADDe;
