...
##############################################################################
.template
conv1d_k3c4s1_template(KERNEL_DMA_LEN=48, OUTPUT_WINDOW_CNT_MINUS_ONE=799, KERNEL_COUNT=16, KERNEL_LOOP_INNER=2, KERNEL_LOOP_OUTER=1):
    # Initialize vector registers for partial sums
    SYS.CTRL (CLEAR.P)
extra_loop:
    LOOPIN $(KERNEL_LOOP_OUTER)  # Loop for $(KERNEL_LOOP_OUTER) kernel sets
...
loop_window:
    ...
    LOOPEND # end of kernel loop
    VPSUM_TSHIFT vp0, K3
    VPSUM_VTSTORE vp1, vt2
    VPSUM vp2
    VPSUM vp3
    LOOPEND # end of input window loop
...
last:
    ...
    LOOPEND # end of kernel set loop
