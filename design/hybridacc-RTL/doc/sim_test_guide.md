...
- `VCS ?= vcs`
- `DW_SIM_VER := <site-specific DesignWare sim library path>`
...
```bash
make sim_noc_sim \
  TEST_DATA_DIR=../../output/noc-sim/gemm \
  VERIFY_TOL=0.05
```
...
```bash
make sim_noc_sim \
  TEST_DATA_DIR=../../output/noc-sim/conv_k3c4 \
  CLOCK_PERIOD_NS=5
```
...
make sim_tb_cluster_sim_advanced \
  TEST_DATA_DIR=../../output/cluster-sim/conv_k3c4ich16och64s
```
...
以下假設目前位於 repo root，以 `test_dma` 為例：

```bash
make -C design/hybridacc-ESL/test/firmware/test_dma clean
make -C design/hybridacc-ESL/test/firmware/test_dma
```
...
```bash
riscv32-unknown-elf-size design/hybridacc-ESL/test/firmware/test_dma/test_dma.elf | awk 'NR==2 {print $1 + $2}'
```

如果要先放到 shell 變數：

```bash
fw_bytes=$(riscv32-unknown-elf-size design/hybridacc-ESL/test/firmware/test_dma/test_dma.elf | awk 'NR==2 {print $1 + $2}')
echo "$fw_bytes"
```
...
```bash
fw_mem=../hybridacc-ESL/test/firmware/test_dma/test_dma.mem
fw_bytes=$(riscv32-unknown-elf-size design/hybridacc-ESL/test/firmware/test_dma/test_dma.elf | awk 'NR==2 {print $1 + $2}')

make -C design/hybridacc-RTL sim_tb_hybridacc_sim \
...
for t in $tests; do
  make -C design/hybridacc-ESL/test/firmware/$t clean >/dev/null
  make -C design/hybridacc-ESL/test/firmware/$t >/dev/null
  fw_bytes=$(riscv32-unknown-elf-size design/hybridacc-ESL/test/firmware/$t/${t}.elf | awk 'NR==2 {print $1 + $2}')
...
|----------|------|--------------|
| Conv1x1 single wave | `design/hybridacc-cc/example/conv1x1/conv2d_1x1_single_wave.yaml` | `output/runtime-check-conv1x1-single` |
| Conv3x3 single wave | `design/hybridacc-cc/example/conv3x3/conv2d_3x3_single_wave.yaml` | `output/runtime-check-conv3x3-single` |
| GEMM single wave | `design/hybridacc-cc/example/gemm/gemm_single_wave.yaml` | `output/runtime-check-gemm-single` |

以下流程請在 `tcsh` shell 中執行；假設目前位於 repo root，並把 `yaml` 與 `out_dir` 換成上表三組之一即可：

```tcsh
set yaml=design/hybridacc-cc/example/conv1x1/conv2d_1x1_single_wave.yaml
set out_dir=output/runtime-check-conv1x1-single
...
- Conv1x1: `yaml=design/hybridacc-cc/example/conv1x1/conv2d_1x1_single_wave.yaml`, `out_dir=output/runtime-check-conv1x1-single`
- Conv3x3: `yaml=design/hybridacc-cc/example/conv3x3/conv2d_3x3_single_wave.yaml`, `out_dir=output/runtime-check-conv3x3-single`
- GEMM: `yaml=design/hybridacc-cc/example/gemm/gemm_single_wave.yaml`, `out_dir=output/runtime-check-gemm-single`
...
```tcsh
make -C design/hybridacc-RTL rtl_regress_gemm_single_wave RTL_FW_DEBUG_PLUSARGS="+TRACE_CLUSTER_RUNTIME +TRACE_CLUSTER_MMIO"
```
...
```tcsh
# 預設靜默模式，不印 trace
make -C design/hybridacc-RTL rtl_regress_gemm_single_wave
...
```tcsh
make -C design/hybridacc-RTL sim_tb_cluster_sim_advanced TEST_DATA_DIR=../../output/cluster-sim/<case>
```
...
cd design/hybridacc-RTL
make sim_noc
make sim_noc_sim TEST_DATA_DIR=../../output/noc-sim/<case>
```
...
cd design/hybridacc-RTL
make sim_cluster
make sim_tb_cluster_sim_advanced TEST_DATA_DIR=../../output/cluster-sim/<case>
```
...
make sim_cluster
make sim_tb_hybridacc_smoke
make sim_noc_sim TEST_DATA_DIR=../../output/noc-sim/<代表case>
make sim_tb_cluster_sim_advanced TEST_DATA_DIR=../../output/cluster-sim/<代表case>
```
...
```bash
ls design/hybridacc-ESL/test/firmware/<case>/<case>.mem
```
