...
| tb_hybridacc_firmware_alu.sv | 已完成（generic firmware harness） | 雖沿用 alu 命名，但已支援以 plusargs 載入不同 firmware image；目前已作為 top-level firmware basic/control-plane 驗證主 harness |
...
| Top-level firmware basic / control-plane | tb/tb_hybridacc_firmware_alu.sv（generic plusarg harness） | 已驗證 PASS：test_alu、test_branch、test_jump、test_loadstore、test_mul、test_stack、test_diag、test_compound、test_hazard、test_sram_timing、test_csr、test_cluster_ctrl、test_fabric、test_dma、test_trap、test_wfi_timer、test_plic |
...
- top-level 已不只 smoke；目前 generic firmware harness 已驗證 17 個 firmware basic / control-plane cases，但仍未進入 GEMM、conv1x1、conv3x3 workload regression。
...
| M5 HybridAcc Top + Workload Bring-up | 大幅完成並驗證 | HybridAcc.sv、tb_hybridacc_smoke.sv 與 generic firmware harness 已存在；目前 17 個 top-level firmware control-plane cases 動態 PASS | 缺 GEMM、conv1x1、conv3x3 tests；NLU path 尚未啟用 |
| M6 Closure | 未開始 | 僅完成 editor diagnostics 等靜態檢查 | 尚無 lint、synthesis、gate-level checks |

## 8. 後續執行順序（由現況出發）

1. 以目前已綠的 control-plane firmware harness 為基礎，往 workload-level regression 推進：GEMM、conv1x1、conv3x3、以及對應結果比對流程。
2. 補齊剩餘 verification gap：Core 細粒度 unit tb、SoC workload tests，以及 Cluster 端更高階 workload / closure 驗證。
3. 決定 CmdToAhbBridge 的定位：要嘛真正接入 CoreController / cluster path，要嘛自 active milestone 移除，避免文件與實作分離。
4. 決定 NLU path 的啟用順序與驗證策略；目前 top 仍以 NUM_NLU=0 tie-off baseline 運作。
5. 補齊 lint、synthesis、selected gate-level checks，使 closure 不只停留在 editor diagnostics 與 RTL sim。
...
| GEMM、conv1x1、conv3x3 可在 RTL 與 ESL 間完成結果比對 | 未滿足 |
| 主要模組具備 lint、synthesis、selected gate-level checks 的可追蹤結果 | 未滿足 |

因此，本輪工作目前最準確的定性是：Cluster bring-up 與現有 Cluster verification ladder 已完成並取得動態 PASS；Core / SoC 的 control-plane firmware bring-up 也已大幅完成，trap/interrupt/WFI/DMA/fabric 類 top-level firmware cases 已動態 PASS，後續主戰場已轉為 workload closure、NLU path 與實體設計前檢查。
