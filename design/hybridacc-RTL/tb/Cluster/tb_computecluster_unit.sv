...
// Module Name:   tb_computecluster_unit
// Project Name:  HybridAcc
// Target Devices: ASIC
// Tool Versions: Synopsys VCS W-2024.09-SP1
// Description:   Smoke-level ComputeCluster integration testbench.
//                Verifies MMIO plumbing for SPM/HDDU/Cluster/NOC and DMA.
...
module tb_computecluster_unit;
    ...
    task automatic cmd_write(input logic [31:0] addr, input logic [31:0] data);
        @(negedge clk);
        cmd_req_addr_i  = addr;
        cmd_req_wdata_i = data;
        cmd_req_wstrb_i = 4'hF;
        cmd_req_write_i = 1'b1;
        cmd_req_valid_i = 1'b1;
        @(posedge clk);
        @(negedge clk);
    ...
    task automatic cmd_read(input logic [31:0] addr, output logic [31:0] data);
        @(negedge clk);
        cmd_req_addr_i  = addr;
        cmd_req_write_i = 1'b0;
        cmd_req_valid_i = 1'b1;
        #(`TB_SETTLE);
        data = cmd_resp_rdata_o;
        @(posedge clk);
        @(negedge clk);
        cmd_req_valid_i = 1'b0;
    endtask
    ...
    initial begin
        ...
        begin
            logic [31:0] rd;
            // SPM config map write/read
            cmd_write(32'h0000_0000, 32'h0000_00E0);
            cmd_read(32'h0000_0000, rd);
            `CHECK_VAL("Cluster cmd readback SPM cfg_map", rd, 32'h0000_00E0)

            // HDDU global read
            cmd_read(32'h0000_1810, rd);
            `CHECK_VAL("Cluster HDDU NUM_PLANES", rd, 32'd4)

            // Cluster mode write/read
            cmd_write(32'h0000_2100, MODE_LAYER_MANAGED);
            cmd_read(32'h0000_2100, rd);
            `CHECK_VAL("Cluster mode readback", rd, MODE_LAYER_MANAGED)

            // Direct NoC command write then read status/cmd mirror
            cmd_write(32'h0000_2000, 32'h0000_0004);
            cmd_read(32'h0000_2000, rd);
            `CHECK_VAL("Cluster noc cmd mirror", rd, 32'h0000_0004)

            // DMA write/read through cluster into SPM
            begin
                logic [63:0] dma_rd;
                dma_write64(32'd16, 64'hABCD_EF01_2345_6789);
                dma_read64(32'd16, dma_rd);
                `CHECK_VAL("Cluster DMA readback", dma_rd, 64'hABCD_EF01_2345_6789)
            end
        end

        `CHECK_COND("Cluster interrupt boolean", interrupt_o === 1'b0 || interrupt_o === 1'b1, interrupt_o)
        `TB_SUMMARY("tb_computecluster_unit")
        $finish;
    end

    initial begin
        #1000000;
        $error("[TB_TIMEOUT] tb_computecluster_unit did not finish in time");
        `TB_SUMMARY("tb_computecluster_unit")
        $finish;
    end
