...
`ifdef GATE_SIM
initial begin
    $sdf_annotate("syn/PErouter/PErouter.sdf", dut);
end
`endif
    ...
    initial begin
        ...
        noc_ps_req_valid = 1; @(negedge clk);
        `CHECK_BIT("CMD_RESET: pe_reset=1", pe_reset, 1'b1)
        `CHECK_BIT("CMD_RESET: pe_start=0", pe_start, 1'b0)
        noc_ps_req_valid = 0;
        ...
        noc_ps_req_valid = 1; @(negedge clk);
        `CHECK_BIT("CMD_START: pe_start=1", pe_start, 1'b1)
        `CHECK_BIT("CMD_START: pe_reset=0", pe_reset, 1'b0)
        noc_ps_req_valid = 0;
        ...
        noc_ps_req_valid = 1; @(negedge clk);
        `CHECK_BIT("LOAD_PROG: pe_program=1", pe_program, 1'b1)
        `CHECK_BIT("LOAD_PROG: im_write_en=1", im_write_en, 1'b1)
        `CHECK_VAL("LOAD_PROG: im_write_addr", im_write_addr, 16'h0123)
        `CHECK_VAL("LOAD_PROG: im_write_data", im_write_data, 16'hABCD)
        noc_ps_req_valid = 0;
        ...
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("PS FIFO: pe_ps_valid=1", pe_ps_valid, 1'b1)
        `CHECK_VAL("PS FIFO: data correct", pe_ps_data, 64'hDEAD_BEEF_1234_5678)
        // Pop it
        pe_ps_ready = 1; @(posedge clk); @(negedge clk);
        ...
        // Pop via resp_ready
        noc_plo_resp_ready = 1; @(posedge clk); @(negedge clk);
        ...
        pe_plo_valid = 1; @(posedge clk); pe_plo_valid = 0;
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("PLO LN: ln_plo_valid=1", ln_plo_valid, 1'b1)
        `CHECK_VAL("PLO LN: data correct", ln_plo_data, 64'h1111_2222_3333_4444)
        ln_plo_ready = 1; @(posedge clk); @(negedge clk);
        ...
        ln_pli_valid = 1; @(posedge clk); @(negedge clk); ln_pli_valid = 0;
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("PLI LN: pe_pli_valid=1", pe_pli_valid, 1'b1)
        `CHECK_VAL("PLI LN: data correct", pe_pli_data, 64'hAAAA_BBBB_CCCC_DDDD)
        pe_pli_ready = 1; @(posedge clk); @(negedge clk);

        // Test 8: Enable=0 blocks all channels
        enable = 0; @(negedge clk);
        `CHECK_BIT("Disable: noc_ps_req_ready=0", noc_ps_req_ready, 1'b0)
        `CHECK_BIT("Disable: noc_pd_req_ready=0", noc_pd_req_ready, 1'b0)
        enable = 1;

        `TB_SUMMARY("tb_perouter")
        $finish;
    end
