...
// Module Name:   tb_exe_m_stage
// Project Name:  HybridAcc
// Target Devices: ASIC
// Tool Versions: Synopsys VCS W-2024.09-SP1
// Description:   Testbench for exe_m_stage module.
// Dependencies:  tb_common.svh, src/hybridacc_utils_pkg.sv, src/PE/TransformRegFile.sv, src/PE/VMULU.sv, src/PE/LDMA.sv, src/PE/SDMA.sv, src/PE/DataMemory.sv, src/PE/EXE_M_Stage.sv
// Revision:
...
`ifdef GATE_SIM
initial begin
    $sdf_annotate("syn/EXE_M_Stage/EXE_M_Stage.sdf", dut);
end
`endif
    ...
    initial begin
        ...
        valid_in = 1;
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("NOP: valid_out=1", valid_out, 1'b1)
        `CHECK_BIT("NOP: ready_out=1", ready_out, 1'b1)
        `CHECK_COND("NOP: no stalls", stall_DL === 1'b0 && stall_PS === 1'b0 && stall_PD === 1'b0, {stall_DL, stall_PS, stall_PD})
        ...
        pd_valid = 1;
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("TR_write: valid_out propagated", valid_out, 1'b1)
        `CHECK_BIT("TR_write: no PD stall (pd_valid=1)", stall_PD, 1'b0)
        pd_valid = 0;
        ...
        pd_valid = 0;
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("PD_stall: stall_PD=1", stall_PD, 1'b1)
        `CHECK_BIT("PD_stall: ready_out=0", ready_out, 1'b0)
        // Unstall
        pd_valid = 1; @(negedge clk);
        `CHECK_BIT("PD_unstall: stall_PD=0", stall_PD, 1'b0)
        // Clear decode_reg to avoid carrying pd_load stall into next test
        ID_decode_signals_in = pe_decode_signals_zero();
        valid_in = 1;
        @(posedge clk); @(negedge clk);
        pd_valid = 0;
        ...
        ps_valid = 0;
        @(posedge clk); @(negedge clk);
        `CHECK_BIT("PS_stall: stall_PS=1", stall_PS, 1'b1)
        ps_valid = 1; @(negedge clk);
        `CHECK_BIT("PS_unstall: stall_PS=0", stall_PS, 1'b0)
        // Clear decode_reg to avoid carrying sys_sdma_act stall into halt test
        ID_decode_signals_in = pe_decode_signals_zero();
        valid_in = 1;
        @(posedge clk); @(negedge clk);
        ps_valid = 0;
        ...
        `CHECK_COND("ReadyBlock: valid_out still previous", valid_out === 1'b0 || valid_out === 1'b1, valid_out)
        ready_in = 1;

        `TB_SUMMARY("tb_exe_m_stage")
        $finish;
    end
