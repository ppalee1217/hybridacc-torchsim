...
`ifdef GATE_SIM
initial begin
    $sdf_annotate("syn/TransformRegFile/TransformRegFile.sdf", dut);
end
`endif
    ...
    initial begin
        ...
        `CHECK_VAL("VecWrite: vtid_out[3]=0xAA99", vtid_out.lanes[3], 16'hAA99)
        ...
        `CHECK_VAL("ShiftK5: reg[3]=0x0304", vtid_out.lanes[1], 16'h0304)
        // reg[6]: mask[6]=1 -> old reg[7] = 0x0307
        `CHECK_VAL("ShiftK5: reg[6]=0x0307", vtid_out.lanes[2], 16'h0307)
        // reg[9]: mask[9]=1 -> old reg[10] = 0x030A
        `CHECK_VAL("ShiftK5: reg[9]=0x030A", vtid_out.lanes[3], 16'h030A)
        ...
        `CHECK_VAL("ShiftK7: reg[0]=0x0401", vtid_out.lanes[0], 16'h0401)
        // reg[3]: mask[3]=1 -> old reg[4] = 0x0404
        `CHECK_VAL("ShiftK7: reg[3]=0x0404", vtid_out.lanes[1], 16'h0404)
        // reg[6]: mask[6]=0 -> 0
        `CHECK_VAL("ShiftK7: reg[6]=0 (cleared)", vtid_out.lanes[2], 16'h0000)
        // reg[9]: mask[9]=0 -> 0
        `CHECK_VAL("ShiftK7: reg[9]=0 (cleared)", vtid_out.lanes[3], 16'h0000)

        `TB_SUMMARY("tb_transformregfile")
        $finish;
    end
