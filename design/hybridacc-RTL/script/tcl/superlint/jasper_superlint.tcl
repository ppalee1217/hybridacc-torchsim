...
#
# Run with:
#   jg -superlint -allow_unsupported_OS design/hybridacc-RTL/script/tcl/superlint/jasper_superlint.tcl
# Run with no GUI:
#   jg -superlint -allow_unsupported_OS -batch design/hybridacc-RTL/script/tcl/superlint/jasper_superlint.tcl
#
...
#   HACC_JG_USE_SRAM_STUBS   Optional 0/1. Default: 1. Compile Jasper-only SRAM macro stubs
#   HACC_JG_BBOX_MODULES     Tcl list of vendor/IP modules to blackbox. Default: {DW_fp_mult DW_fp_add}
#                           plus SRAM macro modules when SRAM stubs are on.
#   HACC_JG_BBOX_MUL_THRESHOLD Optional non-negative integer. Default: 1000.
...
#                           Default: <rtl_root>/script/tcl/superlint/jasper_superlint_waivers.tcl when present.
...
proc hacc_detect_project_root {script_path} {
    ...
    set script_search_dir [file dirname $script_path]
    while {1} {
        if {[hacc_is_project_root $script_search_dir]} {
            return [file normalize $script_search_dir]
        }
        set script_parent [file dirname $script_search_dir]
        if {$script_parent eq $script_search_dir} {
            break
        }
        set script_search_dir $script_parent
    }
...
set use_sram_stubs [hacc_parse_bool HACC_JG_USE_SRAM_STUBS [hacc_init_var HACC_JG_USE_SRAM_STUBS 1]]
set dw_stub_modules {DW_fp_mult DW_fp_add}
set bbox_modules_default $dw_stub_modules
set sram_stub_modules {TS1N16ADFPCLLLVTA128X64M4SWSHOD}
if {$use_sram_stubs} {
    foreach sram_stub_module $sram_stub_modules {
        hacc_unique_append bbox_modules_default $sram_stub_module
    }
}
set bbox_modules [hacc_init_var HACC_JG_BBOX_MODULES $bbox_modules_default]
if {$use_dw_stubs} {
    foreach dw_stub_module $dw_stub_modules {
        hacc_unique_append bbox_modules $dw_stub_module
    }
}
if {$use_sram_stubs} {
    foreach sram_stub_module $sram_stub_modules {
        hacc_unique_append bbox_modules $sram_stub_module
    }
}
...
set waiver_tcl ""
set default_waiver_tcl [file join $project_root script tcl superlint jasper_superlint_waivers.tcl]
...
if {$use_sram_stubs} {
    lappend default_jasper_hdl [file join $project_root script TS1N16ADFPCLLLVTA128X64M4SWSHOD.sv]
}
...
puts " SRAM stubs : $use_sram_stubs"
