...
module HybridAcc import core_pkg::*; #(
    ...
    parameter int unsigned CTRL_ADDR_WIDTH = 32,
    parameter int unsigned CTRL_DATA_WIDTH = 32,
    parameter int unsigned CTRL_STRB_WIDTH = CTRL_DATA_WIDTH / 8,
    parameter int unsigned AXI_ADDR_WIDTH = 32,
    parameter int unsigned AXI_LEN_WIDTH = 8,
    parameter int unsigned AXI_RESP_WIDTH = 2,
    parameter int unsigned HACC_MEM_AXI_DATA_WIDTH = MEM_AXI_DATA_WIDTH
) (
    input  logic clk,
    input  logic reset_n,
    input  logic        s_ctrl_aw_valid_i,
    output logic        s_ctrl_aw_ready_o,
    input  logic [CTRL_ADDR_WIDTH-1:0] s_ctrl_aw_addr_i,
    input  logic        s_ctrl_w_valid_i,
    output logic        s_ctrl_w_ready_o,
    input  logic [CTRL_DATA_WIDTH-1:0] s_ctrl_w_data_i,
    input  logic [CTRL_STRB_WIDTH-1:0] s_ctrl_w_strb_i,
    output logic        s_ctrl_b_valid_o,
    input  logic        s_ctrl_b_ready_i,
    output logic [AXI_RESP_WIDTH-1:0]  s_ctrl_b_resp_o,
    input  logic        s_ctrl_ar_valid_i,
    output logic        s_ctrl_ar_ready_o,
    input  logic [CTRL_ADDR_WIDTH-1:0] s_ctrl_ar_addr_i,
    output logic        s_ctrl_r_valid_o,
    input  logic        s_ctrl_r_ready_i,
    output logic [CTRL_DATA_WIDTH-1:0] s_ctrl_r_data_o,
    output logic [AXI_RESP_WIDTH-1:0]  s_ctrl_r_resp_o,
    output logic        m_mem_axi_aw_valid_o,
    input  logic        m_mem_axi_aw_ready_i,
    output logic [AXI_ADDR_WIDTH-1:0] m_mem_axi_aw_addr_o,
    output logic [AXI_LEN_WIDTH-1:0]  m_mem_axi_aw_len_o,
    output logic        m_mem_axi_w_valid_o,
    input  logic        m_mem_axi_w_ready_i,
    output logic [HACC_MEM_AXI_DATA_WIDTH-1:0] m_mem_axi_w_data_o,
    output logic [HACC_MEM_AXI_DATA_WIDTH/8-1:0] m_mem_axi_w_strb_o,
    output logic        m_mem_axi_w_last_o,
    input  logic        m_mem_axi_b_valid_i,
    output logic        m_mem_axi_b_ready_o,
    input  logic [AXI_RESP_WIDTH-1:0]  m_mem_axi_b_resp_i,
    output logic        m_mem_axi_ar_valid_o,
    input  logic        m_mem_axi_ar_ready_i,
    output logic [AXI_ADDR_WIDTH-1:0] m_mem_axi_ar_addr_o,
    output logic [AXI_LEN_WIDTH-1:0]  m_mem_axi_ar_len_o,
    input  logic        m_mem_axi_r_valid_i,
    output logic        m_mem_axi_r_ready_o,
    input  logic [HACC_MEM_AXI_DATA_WIDTH-1:0] m_mem_axi_r_data_i,
    input  logic [AXI_RESP_WIDTH-1:0]  m_mem_axi_r_resp_i,
    ...
    initial begin
        if ((CTRL_ADDR_WIDTH != 32) || (CTRL_DATA_WIDTH != 32) ||
            (CTRL_STRB_WIDTH != 4) || (AXI_ADDR_WIDTH != 32) ||
            (AXI_LEN_WIDTH != 8) || (AXI_RESP_WIDTH != 2) ||
            (HACC_MEM_AXI_DATA_WIDTH != MEM_AXI_DATA_WIDTH)) begin
            $error("HybridAcc top-level interface widths must match the fixed core-controller and memory AXI contract");
        end
    end
    ...
    logic        cluster_hready_unused[NUM_CLUSTERS];
    logic        cluster_hresp_unused[NUM_CLUSTERS];
    logic [31:0] cluster_hrdata_unused[NUM_CLUSTERS];
    logic        cluster_ahb_unused_reduce;

    always_comb begin
        cluster_ahb_unused_reduce = 1'b0;
        for (int unsigned idx = 0; idx < NUM_CLUSTERS; idx++) begin
            cluster_ahb_unused_reduce ^= cluster_hready_unused[idx]
                                       ^ cluster_hresp_unused[idx]
                                       ^ ^cluster_hrdata_unused[idx];
        end
        cluster_ahb_unused_reduce ^= nlu_cmd_req_valid[0]
                                   ^ nlu_cmd_req_write[0]
                                   ^ (^nlu_cmd_req_addr[0])
                                   ^ (^nlu_cmd_req_wdata[0])
                                   ^ nlu_data_axi_aw_ready
                                   ^ nlu_data_axi_w_ready
                                   ^ nlu_data_axi_b_valid
                                   ^ (^nlu_data_axi_b_resp)
                                   ^ nlu_data_axi_ar_ready
                                   ^ nlu_data_axi_r_valid
                                   ^ (^nlu_data_axi_r_data)
                                   ^ (^nlu_data_axi_r_resp);
    end
    ...
    generate
        for (genvar idx = 0; idx < NUM_CLUSTERS_GEN; idx++) begin : gen_clusters
            ComputeCluster cluster (
                ...
                .hready_i(1'b1),
                .hwdata_i(32'h0),
                .hready_o(cluster_hready_unused[idx]),
                .hresp_o(cluster_hresp_unused[idx]),
                .hrdata_o(cluster_hrdata_unused[idx])
            );
        end
    endgenerate
    ...
    always_comb begin
        ...
        nlu_irq[0] = !(cluster_ahb_unused_reduce === cluster_ahb_unused_reduce);
